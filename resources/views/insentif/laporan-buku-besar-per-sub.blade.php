...
<div class="toolbar no-print">
    <form method="GET" action="{{ route('insentif.laporan-buku-besar-per-sub') }}" style="display:flex; gap:10px; flex-wrap:wrap; align-items:center;">
...
<div class="stat-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); margin-bottom:16px;">
    <div class="stat-card">
        ...
        <div class="value">{{ count($data) }} <span style="font-size:13px; font-weight:normal; color:var(--text-muted);">Bagian/Cabang</span></div>
    </div>
    <div class="stat-card">
...
@forelse ($data as $unitKerja => $group)
    <div class="panel" style="margin-bottom:20px; border-radius:10px; border:1px solid var(--border); overflow:hidden; background:var(--surface);">
        <div style="padding:14px 18px; background:rgba(0,0,0,0.02); border-bottom:1px solid var(--border); display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;">
            <h3 style="margin:0; font-size:15px; font-weight:700;">
                {{ $unitKerja ?: 'Tanpa Unit Kerja' }}
                <span style="font-weight:400; font-size:13px; color:var(--text-muted); margin-left:8px;">({{ count($group['rows']) }} Pegawai)</span>
            </h3>
            <div style="font-size:13px; font-weight:600; display:flex; gap:16px;">
                <span>Bruto: <strong style="color:var(--primary, #0284c7);">Rp {{ number_format($group['total_bruto'], 0, ',', '.') }}</strong></span>
                <span>Potongan: <strong style="color:#ef4444;">Rp {{ number_format($group['total_potongan'], 0, ',', '.') }}</strong></span>
                <span>Bersih: <strong style="color:#10b981;">Rp {{ number_format($group['total'], 0, ',', '.') }}</strong></span>
            </div>
        </div>
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th class="no-print" style="text-align:center; width:80px;">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($group['rows'] as $idx => $d)
                    <tr>
                        <td style="text-align:center; color:var(--text-muted);">{{ $idx + 1 }}</td>
                        <td class="cell-nik"><code>{{ $d['nik'] }}</code></td>
                        <td class="cell-name font-medium">{{ $d['nama'] }}</td>
                        <td style="font-size:12px; color:var(--text-muted);">{{ $d['jabatan'] ?? '-' }}</td>
...
@empty
    <div class="table-card"><div class="table-empty">Belum ada data insentif yang terbit untuk periode ini.</div></div>
@endforelse
...
@media print {
    ...
    .panel { border: none !important; box-shadow: none !important; page-break-inside: avoid; }
