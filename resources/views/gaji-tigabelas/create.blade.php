...
@section('title', 'Proses Gaji 13 Pegawai')

@section('content')
<div class="page-head">
    <div class="breadcrumb">Home / Gaji 13 / Proses Baru</div>
    <h1>Proses Gaji 13 Pegawai</h1>
</div>

<form method="POST" action="{{ route('gaji-tigabelas.store') }}" id="gaji13-form">
    @csrf
    ...
    <div class="form-card" style="max-width:100%; margin-top:16px; background:var(--teal-soft);">
        <div style="display:flex; justify-content:space-between; align-items:center;">
            ...
            <div style="font-family:'IBM Plex Mono',monospace; font-size:22px; font-weight:600; color:var(--teal-dark);" id="gaji13-diterima-display">Rp 0</div>
        </div>
    </div>

    <div class="form-actions" style="max-width:100%;">
        <button type="submit" class="btn btn-primary">Simpan sebagai Draft</button>
        <a href="{{ route('gaji-tigabelas.index') }}" class="btn btn-outline">Batal</a>
    </div>
</form>

<script>
const pegawaiKeluargaUrl = @json(route('gaji-tigabelas.hitung-keluarga', ['pegawaiId' => '__ID__']));
...
function hitungTotal() {
    ...
    document.getElementById('gaji13-diterima-display').textContent = formatRupiah(totalPendapatan - totalPotongan);
}
